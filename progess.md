...
- **2026-05-14:** Updated `vercel.json` to add Cache-Control headers for static assets, addressing Vercel deployment requirements. Created `docs/runbooks.md` containing operational runbooks for incident response, recovery procedures, and an offline mode fallback guide. Implemented a rollback strategy using GitHub Actions in `.github/workflows/rollback.yml`, avoiding command injection by mapping inputs to environment variables. Added operational readiness tests in `backend/ops.test.js` to verify the presence and content of these new operational files. All tests pass successfully. Project completion is approximately 83%.
